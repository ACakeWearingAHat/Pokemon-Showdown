...
class HelpTicket extends Rooms.RoomGame {
	...
	onLogMessage(message, user) {
		if (!this.ticket.open) return;
		if (user.isStaff && this.ticket.userid !== user.userid) {
			if (!this.involvedStaff.includes(user.userid)) this.involvedStaff.push(user.userid);
		}
	...
	close(staff, result) {
		...
		if (!this.involvedStaff.length) {
			if (staff.isStaff && staff.userid !== this.ticket.userid) {
				this.involvedStaff.push(staff.userid);
			} else {
				this.involvedStaff.push(toID(this.ticket.claimed));
			}
		}
	...
	writeStats(result) {
		...
		// Write to TSV
		// ticketType\ttotalTime\ttimeToFirstClaim\tinactiveTime\tresolution\tresult\tstaff,userids,seperated,with,commas
		let line = `${this.ticket.type}\t${(this.closeTime - this.createTime)}\t${firstClaimWait}\t${this.unclaimedTime}\t${this.resolution}\t${this.result}\t${this.involvedStaff.join(',')}`;
		writeStats(line);
	}
...
let commands = {
	...
	helpticket: {
		...
		submit(target, room, user, connection) {
			...
			if (!contexts[ticket.type]) ticket.active = true;
		...
		ban(target, room, user) {
			...
			for (let user of affected) {
				...
				const helpRoom = Rooms(`help-${userid}`);
				if (helpRoom) {
					const ticketGame = /** @type {HelpTicket} */ (helpRoom.game);
